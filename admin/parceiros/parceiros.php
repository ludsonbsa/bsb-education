...
<?php
	...
	if(!empty($_GET['delid'])){
		...
		delete('up_parceiros',"id = '$delId'");

		//Auditoria
		$a['usuario']	= $_SESSION['autUser']['nome'];
		$a['acao'] 		= 'Removeu o parceiro #'.$delId;
...
<section class="content">
		...
		<header>
			...
			<aside>
				<a href="index2.php?exe=parceiros/parceiros-create" style="color:#fff"><button class="green">Criar Parceiro</button></a>
			</aside>
		</header>
    <?php
    $readArt = read('up_parceiros');
	if(!$readArt){
		echo '
				<div class="orange">	
      ...
      <?php
	  	foreach($readArt as $art):
			...
			echo '<tr>';
			echo '<td><img src="../uploads/parceiros/'.$art['thumb'].'" width="55"></img></td>';
			echo '<td>'.$art['nome'].'</td>';
			echo '<td>'.date('d/m/Y H:i:s',strtotime($art['data'])).'</td>';

			echo '<td><a href="index2.php?exe=parceiros/parceiros-edit&editid='.$art['id'].'" title="editar">';
				echo '<img src="ico/edit.png" alt="editar" title="editar" /></a></td>';
			echo '<td><a href="index2.php?exe=parceiros/parceiros&pag='.$pag.'&delid='.$art['id'].'"';
