...
if(function_exists(getUser)){
	if(!getUser($_SESSION['autUser']['id'],'1')){
		...
		$urledit  = $_GET['editid'];
		$readEdit = read('up_clientes',"WHERE id = '$urledit'");
		if(!$readEdit){
			header('Location: index2.php?exe=clientes/clientes');
		}else
...
<section class="content">
	<section class="widget" style="height: 400px; min-height:300px">
		<header>
			...
			<hgroup>
				<h1>Clientes</h1>
				<h2>Editar cliente <strong><?php echo $postedit['nome'];?></strong></h2>
			</hgroup>
			<aside>
				<a href="index2.php?exe=clientes/clientes" style="color:#fff"><button class="blue">Voltar</button></a>
			</aside>
		</header>
<?php
    if(isset($_POST['sendForm'])){
		...
		if(in_array('',$f)){
			...
			if(!empty($_FILES['thumb']['tmp_name'])){
				...
				$a['acao'] 		= 'Editou o cliente '.$f['nome'];
			...
			update('up_clientes',$f,"id = '$urledit'");
			...
			header('Location: index2.php?exe=clientes/clientes-edit&editid='.$urledit);
			}
	...
	?>
		...
		<div class="content">
			...
			<div class="field-wrap">
				<input type="text" name="link" value="<?php echo $postedit['link'];?>" />
			</div>
			...
			<div class="field-wrap">
				<img src="<?php setHome();?>/uploads/clientes/<?php echo $postedit['thumb'];?>" width="80" />
			</div>
