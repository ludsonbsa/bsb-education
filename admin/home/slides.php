...
<?php
	...
	if(!empty($_GET['delid'])){
		...
		delete('up_slides',"id = '$delId'");

		//Auditoria
		$a['usuario']	= $_SESSION['autUser']['nome'];
		$a['acao'] 		= 'Removeu o slide #'.$delId;
...
<section class="content">
		...
		<header>
			...
			<aside>
				<a href="index2.php?exe=home/slides-create" style="color:#fff"><button class="green">Criar Slide</button></a>
			</aside>
		</header>
    <?php
    $readArt = read('up_slides');
	if(!$readArt){
		echo '
				<div class="orange">	
      ...
      <?php
	  	foreach($readArt as $art):
			...
			echo '<tr>';
			echo '<td><img src="../uploads/slides/'.$art['thumb'].'" width="55"></img></td>';
			echo '<td>'.$art['titulo'].'</td>';
			echo '<td>'.lmWord($art['descricao'],150).'</td>';
			echo '<td>'.date('d/m/Y H:i:s',strtotime($art['data'])).'</td>';

			echo '<td><a href="index2.php?exe=home/slides-edit&editid='.$art['id'].'" title="editar">';
				echo '<img src="ico/edit.png" alt="editar" title="editar" /></a></td>';
			echo '<td><a href="index2.php?exe=home/slides&pag='.$pag.'&delid='.$art['id'].'"';
